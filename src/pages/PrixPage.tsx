...
import { ArrowRight, ChevronDown, ChevronLeft, ChevronRight, Headphones, Pause, Phone, Play, ShieldCheck, Sparkles, Users } from "lucide-react";
import "./prix.css";
...
export default function PrixPage() {
  ...
  return <div className="lp-page"><main>
    <section className="lp-video-first" id="presentation"><img className="lp-first-logo" src={logo} alt="Zaki Cleaning Service" /><p className="lp-eyebrow lp-blue lp-video-title">Nettoyage professionnel à Alger</p><div className="lp-video-player"><video ref={videoRef} src={`${base}video/zaki-presentation.mp4`} playsInline preload="metadata" onClick={toggleVideo} onLoadedMetadata={(event) => setDuration(event.currentTarget.duration)} onTimeUpdate={(event) => setCurrentTime(event.currentTarget.currentTime)} onPlay={() => setIsPlaying(true)} onPause={() => setIsPlaying(false)} onEnded={() => setIsPlaying(false)} /><button className={`lp-center-toggle ${isPlaying ? "is-playing" : ""}`} type="button" onClick={toggleVideo} aria-label={isPlaying ? "Mettre la vidéo en pause" : "Lire la vidéo"}>{isPlaying ? <Pause size={27} fill="currentColor" /> : <Play size={27} fill="currentColor" />}</button><div className="lp-video-controls"><div className="lp-video-actions"><span>{formatTime(currentTime)} <small>/</small> {formatTime(duration)}</span></div><input className="lp-progress" type="range" min="0" max={duration || 0} step="0.1" value={currentTime} onChange={(event) => seekVideo(Number(event.target.value))} aria-label="Progression de la vidéo" /></div></div><div className="lp-video-context"><h1>La propreté qui se voit.<br /><em>La confiance qui reste.</em></h1><p>Une équipe sérieuse, un travail soigné et une vraie attention portée à chaque détail de votre espace.</p></div></section>
