...
import { ArrowDown, ArrowRight, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight, Headphones, Phone, Play, ShieldCheck, Sparkles, Users } from "lucide-react";
import "./prix.css";
...
export default function PrixPage() {
  ...
  return <div className="lp-page"><header className="lp-header"><a href="/" aria-label="Retour à Zaki Cleaning"><img src={logo} alt="Zaki Cleaning Service" /></a><a className="lp-header-call" href={phoneHref}><Phone size={16} /> {phone}</a></header><main>
    <section className="lp-hero"><div className="lp-hero-inner"><img className="lp-hero-logo" src={logo} alt="Zaki Cleaning Service" /><p className="lp-eyebrow">Nettoyage professionnel à Alger</p><h1>La propreté qui se voit.<br /><em>La confiance qui reste.</em></h1><p className="lp-hero-text">Une équipe sérieuse, un travail soigné et une vraie attention portée à chaque détail de votre espace.</p><a className="lp-primary" href="#presentation">Découvrir Zaki Cleaning <ArrowDown size={17} /></a></div></section>
    <section className="lp-presentation" id="presentation"><div className="lp-section-intro"><p className="lp-eyebrow lp-blue">Découvrez notre méthode</p><h2>Un nettoyage professionnel, pensé pour vous.</h2><p>De la préparation au contrôle final, nous organisons chaque intervention avec le matériel adapté et une équipe attentive.</p></div><div className="lp-video-placeholder"><div className="lp-play"><Play size={24} fill="currentColor" /></div><p>Votre vidéo de présentation</p><span>Elle sera ajoutée ici prochainement</span></div></section>
