...
const audioReviews = [
  ...
  { file: "voix-11.ogg", label: "Témoignage client 11", type: "audio/ogg" },
  { file: "voix-12.ogg", label: "Témoignage client 12", type: "audio/ogg" },
];

const faq = [
  { question: "Dans quelles zones intervenez-vous ?", answer: { intro: "Zaki Cleaning intervient auprès des particuliers, entreprises et établissements professionnels dans les wilayas suivantes :", items: ["Alger", "Blida", "Boumerdès"] } },
  { question: "Quels types de nettoyage proposez-vous ?", answer: ["Nettoyage de fin de chantier : maisons, villas, usines et administrations", "Offre spéciale pour les promoteurs", "Nettoyage profond", "Nettoyage après location", "Nettoyage après incendie, sinistre ou inondation", "Service de femmes de ménage", "Entretien des espaces verts", "Nettoyage administratif quotidien", "Nettoyage de restaurants, bureaux, écoles et cliniques"] },
  { question: "Quels sont vos tarifs ?", answer: { intro: "Nos prestations démarrent à partir de 25 000 DA. Le tarif final est établi selon :", items: ["L’état du lieu à nettoyer", "La superficie", "La localisation", "Le nombre de personnes nécessaires selon l’ampleur du nettoyage", "Le niveau de nettoyage demandé : nettoyage profond et minutieux ou nettoyage de surface"] } },
...
export default function PrixPage() {
  ...
  return <div className="lp-page"><main>
    ...
    <section className="lp-faq"><div className="lp-section-intro"><p className="lp-eyebrow lp-blue">Questions fréquentes</p><h2>Vous voulez en savoir plus ?</h2></div><div className="lp-faq-list">{faq.map((item, index) => <article key={item.question} className={openFaq === index ? "open" : ""}><button type="button" aria-expanded={openFaq === index} onClick={() => setOpenFaq(openFaq === index ? null : index)}><span>{item.question}</span><ChevronDown /></button><div className="lp-faq-answer">{Array.isArray(item.answer) ? <ul>{item.answer.map((service) => <li key={service}>{service}</li>)}</ul> : typeof item.answer === "object" ? <div className="lp-faq-list-content"><p>{item.answer.intro}</p><ul>{item.answer.items.map((detail) => <li key={detail}>{detail}</li>)}</ul></div> : <p>{item.answer}</p>}</div></article>)}</div></section>
